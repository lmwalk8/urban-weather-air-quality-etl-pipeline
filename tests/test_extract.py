...
def test_extract_data_column_values(extract_data):
    ...
    expected_column_names = [
        ...
        'Temp_Range', 'Heat_Index', 'Severity_Score', 'Condition_Code',
        'Month', 'Season', 'Day_of_Week', 'Is_Weekend', 'Health_Risk_Score'
    ]
    assert df.columns.to_list() == expected_column_names, "Actual column names do not match expected."
...
def test_extract_data_city_names(extract_data):
    ...
    assert df["City"].unique().tolist() == expected_city_names, "Actual city names do not match expected."
